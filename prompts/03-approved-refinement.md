...
2. Read `AGENTS.md`, all project documents, the complete audit, the `Keep unchanged` list, and the exact approved findings.
...
7. Do not invent or strengthen business information. If an approved finding requires a fact that remains unconfirmed, stop that item and report the evidence needed.
8. Keep `README.md`, `project.md`, `audit/`, `assets/`, and `source/` unchanged unless an approved finding explicitly names an exact permitted change. Never treat approval of a site finding as permission to reorganize the repository.
9. Do not overwrite an existing source/reference screenshot. Add final screenshots only after implementation and QA, using descriptive new filenames.
10. If `site/css/input.css` changes, confirm the Tailwind standalone executable and exact command, rebuild `site/css/styles.css`, and verify the compiled output corresponds to the source. The build command is currently undocumented; do not guess.
11. Test every affected success, failure, hover, focus, keyboard, menu, and reduced-motion state that applies.
12. Recheck the full page at approximately 375, 390, 768, 1024, and 1440 pixels, including regression checks outside the directly edited viewport.
13. Run the complete `docs/06-qa-checklist.md`, recording evidence and every item that cannot be verified or is not applicable.
14. Produce final desktop and mobile screenshots only when the page meets the approved prospect presentation milestone.
15. Update `docs/00-project-status.md`, append new decisions to `docs/05-build-decisions.md`, and update `docs/10-definition-of-done.md` only with evidence actually obtained.
16. Review the complete diff for scope creep, protected-file changes, unsupported claims, unapproved copy changes, secrets, placeholders, temporary artifacts, and accidental asset replacement.
17. Commit the approved refinement pass with a clear message. Do not merge it.
